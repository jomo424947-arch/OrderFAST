...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center bg-surface/50 border border-dashed border-line rounded-3xl my-6">
      <div className="w-16 h-16 rounded-2xl bg-surface border-2 border-line/90 flex items-center justify-center text-primary mb-4 shadow-xs">
        {icon || <PackageOpen className="w-8 h-8 stroke-[2.2]" />}
      </div>
      <h4 className="font-display font-bold text-lg text-ink mb-1.5">{title}</h4>
      {description && (
        <p className="font-body text-xs sm:text-sm text-ink-soft font-medium max-w-sm mb-5 leading-relaxed">
          {description}
        </p>
      )}
